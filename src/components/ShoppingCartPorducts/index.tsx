...
function ShoppingCartPorducts() {
  ...
  const decrease = (product: IShoppingCartProduct) => {
    const newProduct = {
      ...product,
  ...
  const increase = (product: IShoppingCartProduct) => {
    const newProduct = { ...product, quantity: product.quantity + 1 };
    dispatch(updateShoppingCartProduct(newProduct));
  };

  const deleteProductFromCart = (product: IShoppingCartProduct) => {
      ...
      {shoppingCartProducts.length > 0 && (
        <ShoppingContainer $display={shoppingCartPanel}>
          ...
          <div className="list-products-details">
            {shoppingCartProducts.map((product: IShoppingCartProduct) => (
